...
# 一个简单的圆点 Dot (m0)
# 一个带有注释的圆点 AnnotationDot (m1)
# 一个带有标签的圆点 LabeledDot (m2)
# 一个带有数学公式标签的圆点 LabeledDot (m3)
# 一个曲线箭头 CurvedArrow (m4)
# 一个带有负半径的曲线箭头 CurvedArrow (m5)
# 一个双曲线箭头 CurvedDoubleArrow (m6)
...
# 然后将这些对象添加到场景中，并通过循环将它们沿垂直方向向下移动，以便在屏幕上垂直排列。
...
# 当使用类似from module_name import *的语法时，只会导入__all__列表中指定的符号，而不会导入其他未在列表中指定的符号
# 这样可以避免导入过多的符号
__all__ = [
    "TipableVMobject",
    "Arc",
...
# 这些常量定义了默认的点和箭头的尺寸参数：
# DEFAULT_DOT_RADIUS：默认点的半径大小为 0.08。
# DEFAULT_SMALL_DOT_RADIUS：默认小点的半径大小为 0.04。
# DEFAULT_DASH_LENGTH：默认虚线的长度为 0.05。
# DEFAULT_ARROW_TIP_LENGTH：默认箭头的长度为 0.35。
# DEFAULT_ARROW_TIP_WIDTH：默认箭头的宽度为 0.35。
...
class TipableVMobject(VMobject, metaclass=ConvertToOpenGL):
    ...
    添加、创建、修改 tips
        add_tip 方法调用 create_tip 方法, 在将新 tip 推入 TipableVMobject 的子对象列表之前
        进行风格和位置配置
    ...
    # 移除并返回 TipableVMobject 实例中的箭头(tip)
    def pop_tips(self) -> VGroup:
        # 获取 TipableVMobject 实例的起始点和终点
        start, end = self.get_start_and_end()
        # 存储被移除的箭头
        result = self.get_group_class()()
        # 如果 TipableVMobject 实例有箭头，将箭头添加到 result 中，并从实例中移除箭头
        if self.has_tip():
            result.add(self.tip)
            self.remove(self.tip)
        # # 如果 TipableVMobject 实例有起始箭头，将起始箭头添加到 result 中，并从实例中移除起始箭头
        if self.has_start_tip():
            result.add(self.start_tip)
            self.remove(self.start_tip)
        # 最后，将起始点和终点重新放置到原来的位置，并返回存储了被移除箭头的 result
